...
const services=[
  ...
  {num:"05",title:"Printbeheer",text:"Krijg inzicht in apparatuur, verbruik en tellerstanden en maak beheer voorspelbaarder."},
  {num:"06",title:"Optimalisatie",text:"Uw organisatie verandert. Daarom blijven we kijken of apparatuur, software en werkwijze nog aansluiten."},
];
...
export default function Services(){
 return <main>
  ...
  <section className="container editorial-section"><div className="eyebrow">VAN START TOT DAGELIJKS GEBRUIK</div><h2>Één aanpak voor de hele levenscyclus.</h2><p className="lead">Van het eerste advies tot onderhoud en optimalisatie: we houden de oplossing praktisch, persoonlijk en overzichtelijk.</p><div className="service-grid">{services.map(s=><article className="service-card" key={s.num}><div className="service-number">{s.num}</div><h3>{s.title}</h3><p>{s.text}</p><Link className="text-link" href="/contact">Bespreek dit →</Link></article>)}</div></section>
